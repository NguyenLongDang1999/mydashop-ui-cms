...
<template>
    ...
    <BaseDataTableInit
        v-slot="{ row, column }: IRow<IProductAttribute>"
    ...
    >
        ...
        <UToggle
            v-if="areValuesEqual(column.key, PRODUCT_ATTRIBUTE_KEYS.STATUS)"
            :model-value="areValuesEqual(row.status, STATUS.ACTIVE)"
        />

        <span v-if="areValuesEqual(column.key, PRODUCT_ATTRIBUTE_KEYS.CREATED_AT)">
